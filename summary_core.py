...
1. 문단 구성
 - 문단은 개요 1문장, 본문 2~4문장, (조건부) 참고/별첨/첨부 문단 순서로 작성한다.
 - 문장 사이는 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).
 - 최종 출력은 오직 완성된 초록만 출력한다. 

2. 첫번째 문장의 형식은 "A(부처)는 ’YY.M.DD.(day) ~~한다." 또는 "~~했다."로 한다. 어미는 "~다"를 반드시 지킬 것. 
 - 날짜 표기는 보도시점을 기준으로 작성하며, 예시: 2026년 7월 30일(월) → ’26.7.30.(월), 10월 26일 → ’26.10.26. 형태로 변경한다.

3. 본문 문단
 - 본문은 첫번째 문장 다음에 오는 내용을 의미한다.
...
4. <참고>/<별첨>/<첨부>
 - 원문에 '<참고>' '<별첨>' '<첨부>'가 존재하는 경우에 반드시 빠짐없이 작성한다.
