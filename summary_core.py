...
1. 문단 구성
 - 문단은 개요 1문장, 본문 2~4문장, (조건부)참고/붙임/별첨 문단 순서로 작성한다.
 - 문장 사이는 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).
 - 최종 출력은 오직 완성된 초록만 출력한다. 

2. 첫 번째 개요 문장의 형식은 "A(부처)는 ’YY.M.DD.(day) ~한다." 또는 "~했다."로 한다. 
 - 어미는 "~다"를 반드시 지킬 것. 
 - 날짜 표기는 보도시점을 기준으로 작성하며, 예시: 2026년 7월 30일(월) → ’26.7.30.(월), 10월 26일 → ’26.10.26. 형태로 변경한다.

3. 본문 문단
 - 본문은 첫 번째 문장 다음에 오는 내용을 의미한다.
 - 본문의 어미만 음슴체로 끝내고 마지막에 "." 를 찍는다.
 - 한 문단은 한 문장으로만 구성하며, 절대로 두 문장으로 작성하지 않는다.
 - 날짜 표기는 예: 7월 30일(월) → 7.30.(월), 10월 26일 → 10.26. 형태로 변경한다.
 - 개요를 제외한 모든 본문 문단 앞에는 "-"를 붙인다.
 - 주요 내용을 간결하게 요약하되, 해당 자료에서 말하고자 하는 바와 강조점은 명확히 포함한다.
 - 본문은 가급적 2~4문장 이내로 요약한다.
 - <참고>, <붙임>, <별첨> 등 추가 자료 이전까지의 내용만 본문 초록 작성에 사용하지 않는다.
 - 각 문장은 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).

4. <참고>/<붙임>/<별첨>
 - 원문에 '<참고>' '<붙임>' '<별첨>'이 존재하는 경우, 반드시 빠짐없이 작성한다.
 - 반드시 '<참고>' '<붙임>' '<별첨>'바로 뒤에 오는 원문에 사용된 제목만 있는 그대로 작성하고, 그 외의 내용은 작성하지 않는다.
 - 원문에 사용된 표기(<참고>, <붙임>, <별첨>)와 번호를 변경 없이 그대로 유지한다.
 - 여러개가 있는 경우에도 하나의 문단으로 작성한다.
 - 작성 형식은 아래와 같다.
 ...
 - 표제(<참고>/<붙임>/<별첨>)는 한 번만 작성한다.
 - 하위 항목 나열은 숫자 번호(1., 2., …)만 사용한다.
 - 하위 항목이 하나인 경우는 번호 없이 전부 한 줄로 작성한다. 예: <참고> 원문에 사용된 제목
...
5. 작성 원칙
 ...
 - 모든 출력 텍스트는 Unicode(UTF-8) 기준으로 처리하며, 원문에 포함된 문자(한글, 기호, 따옴표, 특수문자 등)가 `?` 등으로 치환되거나 손실되지 않도록 그대로 유지한다.


6. 이곳에 첨부한 PDF 자료(정부 보도자료)를 활용해 위 지침을 모두 따른 하나의 완성된 초록을 작성한다.
...
1. 문단 구성
 - 문단은 개요 1문단, 본문 2~4문단, 마무리 1문단, (조건부) 참고/별첨/첨부 문단 순서로 작성한다.
 - 문단 사이는 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).
 - 최종 출력은 오직 완성된 초록만 출력한다. 

2. 개요 문장은 한 문장으로 작성한다.
 - 개요 문단은 보도자료 양식의 첫 문단을 의미한다. 
 - 형식: "A(부처)는 ’YY.M.DD.(day) ~~한다고 밝혔다." 또는 "~~했다."
 - 날짜 표기는 보도시점을 기준으로 작성하며, 예시: 2026년 7월 30일(월) → ’26.7.30.(월), 10월 26일 → ’26.10.26. 형태로 변경한다.

3. 본문 문단
 - 본문 문단은 보도자료의 첫 문단과 마지막 문단을 제외한 본문 문단들을 의미한다. 
 - 어미는 모두 음슴체로 끝내고 마지막에 "." 를 찍는다.
 - 한 문단은 한 문장으로만 구성하며, 절대로 두 문장으로 작성하지 않는다.
 - 날짜 표기는 예: 7월 30일(월) → 7.30.(월), 10월 26일 → 10.26. 형태로 변경한다.
 - 개요를 제외한 모든 본문 문단 앞에는 "-"를 붙인다.
 - 주요 내용을 간결하게 요약하되, 해당 자료에서 말하고자 하는 바와 강조점은 명확히 포함한다.
 - 본문은 가급적 2~4문단 이내로 요약한다.
 - <참고><별첨><첨부> 등 추가 자료의 내용은 본문 초록 작성에 사용하지 않는다.
 - 각 문단은 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).
 
4. 마무리 문장은 A가 향후 ~~할 계획이거나 ~~하겠다고 밝혔다는 내용을 포함한 음슴체 문장으로 작성.
 - 마무리 문장은 보도자료의 마지막 문단(끝에서 1~2문단)을 의미한다.
...
5. <참고>/<별첨>/<첨부>
 ...
 - 하위 항목이 하나인 경우는 번호 없시 작성한다. 예: <참고> 원문에 사용된 제목
 - 해당 블록 전체 앞뒤로만 문단 구분(빈 줄 1줄)을 둔다.
...
1. 문단 구성
 ...
 - 문단 사이는 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).
 - 최종 출력은 오직 완성된 초록만 출력한다. 
 - 아래에 언급하는 개요 문장은 보도자료의 첫 문단을 의미, 본문은 보도자료의 첫 문단, 마지막 문단을 제외한 내용을 의미한다.
 - 마무리 문장은 보도자료의 마지막 문단을 의미한다. 

2. 개요 문단은 한 문장으로 작성한다.
 - 형식: "A(부처)는 ’YY.M.DD.(day) ~~다." 또는 "~~했다."로 작성한다. 다만, 첨부된 보도자료 원문에서 “~라고 밝혔다”라고 명시된 것은 유지한다. 
 - 날짜 표기는 보도시점을 기준으로 작성하며, 예시: 2026년 7월 30일(월) → ’26.7.30.(월), 10월 26일 → ’26.10.26. 형태로 변경한다.
...
"""
PROMPT_5 = """
정부 보도자료의 초록을 아래의 지침에 따라 작성해 주세요.

1. 문단 구성
 - 문단은 개요 1문장, 본문 2~4문장, 마무리 1문장, (조건부)참고/별첨/첨부 순서로 작성한다.
 - 문단 사이는 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).
 - 최종 출력은 오직 완성된 초록만 출력한다.

2. 개요 1문장
 - 의미: 개요 문장은 보도자료에서 첫 문단을 의미한다.
 - 형식: “A(부처)는 ’YY.M.DD.(day) -ㄴ다.” 형식의 평서문(~다.)으로 작성한다.
 - 날짜 표기는 “보도시점”을 기준으로 작성하며, 예시: 2026년 7월 30일(월) → ’26.7.30.(월), 10월 26일 → ’26.10.26. 형식로 변경한다.

3. 본문 2~4문장
 - 의미: 본문 문단은 보도자료의 첫 문단과 마지막 문단을 제외한 본문 문단들을 의미한다. 
 - 본문 형식
   · 본문 문단 앞에 "-"를 붙인다.
   · 한 문단은 한 문장으로만 구성하며, 절대로 두 문장으로 작성하지 않는다.
   · 본문 문장의 어미는 모두 음슴체로 끝내고 문장 끝에 "." 를 찍는다.
   · 날짜 표기는 예: 7월 30일(월) → 7.30.(월), 10월 26일 → 10.26. 형태로 변경한다.
   · <참고>, <별첨>, <첨부> 등 추가 자료의 내용은 본문 초록 작성에 사용하지 않는다.
   · 각 문단은 반드시 빈 줄 1줄로만 구분한다(즉 '\n\n' 형태).
   · 보도자료의 첫 문단과 마지막 문단을 제외한 나머지 본문 문단들을 2~4문장 이내로 요약하여 초록 본문 1문단을 만든다.
   · 주요 내용을 간결하게 요약하되, 해당 보도자료에서 말하고자 하는 바와 강조점은 명확히 포함한다.
 
4. 마무리 1문장
 - 마무리 1문장 앞에 "-"를 붙인다.
 - “A가 향후 ~할 계획이다” 또는 “~하겠다”고 밝혔히는 내용을 포함한 음슴체 한 문장으로 작성.
 - A(부처)의 직책과 개인 이름은 쓰지 않고 A(부처)만 작성한다.

5. <참고>/<별첨>/<첨부>
 - 보도자료에 '<참고>' '<별첨>' '<첨부>'가 존재하는 경우에만 작성한다.
 - 보도자료에 사용된 표기(<참고>, <별첨>, <첨부>)와 번호를 변경 없이 그대로 유지한다.
 - 여러 개가 있는 경우에도 하나의 문단으로 작성한다.
 -  '<참고>' '<별첨>' '<첨부>' 자료안에 ‘<>’ 형식의 문구는 작성하지 않는다.
 - 작성 예시는 아래와 같다.
   <참고>
   1. 보도자료에 사용된 제목 1
   2. 보도자료에 사용된 제목 2
 ...
 - 표제(<참고>/<별첨>/<첨부>)는 한 번만 작성한다.
 - 하위 항목은 숫자 번호(1., 2., …)만 사용한다.
 - 하위 항목이 하나인 경우는 번호 없이 작성한다. 예: <참고> 원문에 사용된 제목
 - 해당 블록 전체 앞,뒤로만 문단 구분(빈 줄 1줄)을 둔다.
