...
1. 문단 구성은 개요 한 문장 - 본문(최대 5문단) - 마무리 문장

2. 개요 한 문장은 반드시 다음과 같은 형식을 따라야 함.
  - 개요 한 문장: A(부처)는 MM.DD.(day) ~~한다고 밝혔다. or 했다.
  - 날짜는 예를 들면, 7월 30일(월) → 7.30.(월) 형식으로 변경(10월 26일 → 10.26.), 이는 초록 전체에 적용

3. 본문: 
  - 어미를 음슴체 또는 명사형으로 끝내고, 문장이 어색하게 끝나면 안됨.
  - 주요내용을 간단히 요약하되, 해당 자료에서 말하고자 하는 바, 강조하고자 하는 바는 명확히 포함되어야 함. 
  - 주어와 서술의 호응이 완벽해야 하며, 절대로 비문이 작성되면 안됨. 
  - 가급적 5문단 안에 요약해야 하며, 문서에 따라 유연하게 적용하되(7~8문단도 가능은 하나 최소한으로 해야 함), 3~4문단이 적정함.
  - 한 문단은 한 문장(2문장도 가능은 하나 최소로 할 것)으로 구성되며, 개요 한 문장을 제외한 각 문단과 마무리 문장 앞에 "-" 추가 
  - "-" 는 엔터로 한 줄 더 띄워서 문장이 구분되게 작성

4. 마무리 문장: A가 ~~할 계획이거나 ~~하겠다고 밝혔다는 내용을 포함한 음슴체 문장
 - A(부처)의 직책과 이름은 생략하고 A(부처)만 작성
...
6. 모든 문장은 말이 되어야 하며, 맥락 상 어색하지 않아야 함. 또한, 모든 단어 및 문장은 해당 보도자료 텍스트만을 활용해야 하며, 임의로 자료 외 외부 내용을 포함하면 안됨.

7. 이곳에 첨부한 PDF 자료(정부 보도자료)를 활용하여 모두 위 지침을 따른 하나의 완성된 초록으로 만들어줘야 함.

8. 위 프롬프트의 조건들은 빠짐없이 지켜서 작성되어야 함 
